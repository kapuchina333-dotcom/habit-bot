        ...
        if ok:
            ...
            kb.append([
                InlineKeyboardButton(
                    "✅", callback_data=f"t_{hid}"),
                InlineKeyboardButton(
                    "⏱", callback_data=f"m_{hid}")
            ])
        else:
            t += f"⬜ {hn}\n"
            kb.append([
                InlineKeyboardButton(
                    f"⬜ {hn}", callback_data=f"t_{hid}")
            ])
    tot = len(rows)
...
async def calendar(msg, u, wo=0, ed=False):
    ...
    for i in range(7):
        ...
        mark = " ← СЕГОДНЯ" if ds == td else ""
        total = r(
            "SELECT COUNT(*) FROM h "
            "WHERE u=? AND a=1", (u,), 1)
        done = r(
            "SELECT COUNT(*) FROM c "
            "WHERE u=? AND d=?", (u, ds), 1)
    ...
    kb = [[
        ...
        InlineKeyboardButton("➡️", callback_data=n)
    ]]
    mk = InlineKeyboardMarkup(kb)
    if ed:
        try: await msg.edit_text(t, reply_markup=mk)
...
async def cb(up, ctx):
    ...
    if d.startswith("t_"):
        ...
        x = r("SELECT id FROM c WHERE h=? AND d=?",
              (hid, dd), 1)
        if x:
            r("DELETE FROM c WHERE id=?", (x[0],))
        else:
            r("INSERT INTO c(u,h,d,m) VALUES(?,?,?,0)",
              (u, hid, dd))
        ...
        x = r("SELECT id FROM c WHERE h=? AND d=?",
              (hid, dd), 1)
        if x:
            r("UPDATE c SET m=? WHERE id=?",
              (mn, x[0]))
        else:
            r("INSERT INTO c(u,h,d,m) VALUES(?,?,?,?)",
              (u, hid, dd, mn))
...
async def txt(up, ctx):
    ...
    if "Сегодня" in t:
        ...
        hb = r("SELECT id,n FROM h WHERE u=? AND a=1",
               (u,), 2)
        if not hb:
            await up.message.reply_text(
                "Нет!", reply_markup=MN)
        ...
        for i, n in hb:
            kb.append([InlineKeyboardButton(
                f"🗑 {n}", callback_data=f"del_{i}")])
        await up.message.reply_text(
            "🗑 Какую?", reply_markup=InlineKeyboardMarkup(kb))
    elif "Стат" in t:
        hb = r("SELECT id,n FROM h WHERE u=? AND a=1",
               (u,), 2)
        if not hb:
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND, txt))
    print("OK")
    app.run_polling()
