...
def stk(u,hid):
 s=0;d=datetime.now()
 dc=q("SELECT days FROM h WHERE id=?",(hid,),f1=True)
...
def ttl(u):
 r=q("SELECT COUNT(*) FROM c WHERE uid=?",(u,),f1=True)
...
def gmt(u,hid):
 s=datetime.now().replace(day=1).strftime("%Y-%m-%d")
...
def bpk(u):
 ...
 kb.append([B("✅ Готово",callback_data="dp")])
 return kb
...
async def cmd_start(up,ctx):
 u=up.effective_user.id;cs(ctx)
 q("INSERT OR IGNORE INTO u(uid,un)VALUES(?,?)",(u,up.effective_user.username or ""))
...
async def show_today(msg,u,edit=False):
 ...
 for hid,hn,comp,mins in st:
  if comp:
   s=stk(u,hid);line="✅ "+hn
   if s>1:line+=" 🔥"+str(s)
   if mins>0:line+=" ("+fm(mins)+")"
   dn+=1
  else:
   line="⬜ "+hn
  t+=line+"\n"
  row=[B("✅" if comp else "⬜",callback_data="t_"+str(hid))]
  if comp:
   row.append(B("⏱ Записать время",callback_data="mn_"+str(hid)))
  kb.append(row)
 tot=len(st);pct=round((dn/tot)*100) if tot else 0
 t+="\n"+"🟩"*(pct//10)+"⬜"*(10-pct//10)+" "+str(pct)+"% ("+str(dn)+"/"+str(tot)+")"
 if dn==tot and tot>0:t+="\n\n🎉 ИДЕАЛЬНЫЙ ДЕНЬ!"
 if edit:
  try:await msg.edit_text(t,reply_markup=M(kb))
  except:pass
 else:await msg.reply_text(t,reply_markup=M(kb))

async def cmd_today(up,ctx):
 cs(ctx);await show_today(up.message,up.effective_user.id)
...
async def show_cal(msg,u,wo=0,edit=False):
 ...
 for i in range(7):
  ...
  dc=sum(1 for hid,_,_ in dh if q("SELECT id FROM c WHERE hid=? AND date=?",(hid,ds),f1=True))
  tl=len(dh)
  if tl==0:sv="—"
  elif day.date()<=today.date():
   if dc==tl:sv="✅"+str(dc)+"/"+str(tl)
   elif dc>0:sv="⚠️"+str(dc)+"/"+str(tl)
   else:sv="❌0/"+str(tl)
  else:sv="📌"+str(tl)
  t+=DR[day.weekday()]+" "+day.strftime("%d.%m")+": "+sv+it+"\n"
 kb=[[B("⬅️",callback_data="cl_"+str(wo-1)),B("Сегодня",callback_data="clmt(u,hid)
  tx="⏱ "+(hn[0] if hn else "?")+"\n📅 За месяц: "+fm(cur)+"\n\nВыбери время:"
  kb=[
   [B("15 мин",callback_data="sm_"+str(hid)+"_15"),B("30 мин",callback_data="sm_"+str(hid)+"_30"),B("45 мин",callback_data="sm_"+str(hid)+"_45")],
   [B("1 ч",callback_data="sm_"+str(hid)+"_60"),B("1.5 ч",callback_data="sm_"+str(hid)+"_90"),B("2 ч",callback_data="sm_"+str(hid)+"_120")],
   [B("3 ч",callback_data="sm_"+str(hid)+"_180"),B("4 ч",callback_data="sm_"+str(hid)+"_240"),B("5 ч",callback_data="sm_"+str(hid)+"_300")],
   [B("✍️ Своё число",callback_data="ct_"+str(hid))],
   [B("⬅️ Назад",callback_data="bk")]
  ]
  ...
  try:await cq.message.edit_text("✍️ Напиши минуты:\n90 = полтора часа\n1:30 = 1 час 30 мин")
  except:pass
 elif d=="bk":
  cs(ctx);await show_today(cq.message,u,edit=True)
 elif d.startswith("cl_"):
  await show_cal(cq.message,u,int(d[3:]),edit=True)
 elif d=="s7" or d=="s30":
  ...
  for i in range(days):
   ...
   cn=q("SELECT COUNT(*) FROM h WHERE uid=? AND act=1 AND days LIKE '%'||?||'%'",(u,w),f1=True)
   tp+=cn[0] if cn else 0
   r=q("SELECT COUNT(*) FROM c WHERE uid=? AND date=?",(u,ds),f1=True)
   td+=r[0] if r else 0
  ...
  q("UPDATE u SET rh=-1 WHERE uid=?",(u,))
  try:await cq.message.edit_text("🔕 Выключено!")
  except:pass
 elif d=="rc":
  ctx.user_data["rc"]=True
  try:await cq.message.edit_text("✍️ Напиши время (20:30)")
  except:pass
  ...
  try:await cq.message.edit_text("📋 Выбери:",reply_markup=M(bpk(u)))
  except:pass
 elif d=="ra":
  try:await cq.message.edit_text("💣 Точно удалить ВСЁ?",reply_markup=M([[B("✅ Да",callback_data="ry"),B("❌ Нет",callback_data="rn")]]))
  except:pass
 elif d=="ry":
  for tb in["c","h","u"]:q("DELETE FROM "+tb+" WHERE uid=?",(u,))
  try:await cq.message.edit_text("🗑 Удалено! /start")
  except:pass
 elif d=="rn":
  try:await cq.message.edit_text("👍 Отменено!")
  except:pass
 elif d.startswith("dh_"):
  hid=int(d[3:]);r=q("SELECT name FROM h WHERE id=?",(hid,),f1=True)
  q("UPDATE h SET act=0 WHERE id=?",(hid,))
...
async def handle_txt(up,ctx):
 ...
 if ctx.user_data.get("ct"):
  try:
   ...
   if ":" in tx:
    m=int(tx.split(":")[0])*60+int(tx.split(":")[1])
   else:
    m=int(tx)
   hid=ctx.user_data["ct"];stm(u,hid,m);cs(ctx)
   await up.message.reply_text("✅ "+fm(m),reply_markup=MN)
  except:
   await up.message.reply_text("❌ Напиши число! 90 или 1:30")
  return
 if ctx.user_data.get("rc"):
  try:
   p=t.replace(".",":").split(":");h=int(p[0]);m=int(p[1]) if len(p)>1 else 0
   if 0<=h<=23 and 0<=m<=59:
    q("UPDATE u SET rh=?,rm=? WHERE uid=?",(h,m,u));cs(ctx)
    await up.message.reply_text("🔔 "+str(h)+":"+str(m).zfill(2)+" ✅",reply_markup=MN)
   else:
    await up.message.reply_text("❌ 0:00-23:59")
  except:
   await up.message.reply_text("❌ Напиши 20:30")
  return
 if ctx.user_data.get("adding"):
  q("INSERT INTO h(uid,name,days)VALUES(?,?,?)",(u,t,"0123456"));cs(ctx)
  await up.message.reply_text("✅ "+t+" добавлена!",reply_markup=MN)
  return
 if "Сегодня" in t:cs(ctx);await show_today(up.message,u)
  ...
  cs(ctx);ctx.user_data["adding"]=True
  await up.message.reply_text("✍️ Напиши название:")
 elif "Удалить" in t:
  hb=gh(u)
  if not hb:await up.message.reply_text("Нет привычек!",reply_markup=MN);return
  await up.message.reply_text("🗑 Какую?",reply_markup=M([[B("🗑 "+hn,callback_data="dh_"+str(hid))] for hid,hn in hb]))
 elif "Достижения" in t:await cmd_ach(up,ctx)
 elif "Помощь" in t:await cmd_help(up,ctx)
 else:await up.message.reply_text("Меню 👇",reply_markup=MN)

def main():
 init_db();print("ok")
 app=Application.builder().token(BOT_TOKEN).post_init(post_init).build()
 for c,f in[("start",cmd_start),("today",cmd_today),("stats",cmd_stats),("reminder",cmd_rem),("settings",cmd_set),("achievements",cmd_ach),("calendar",cmd_cal),("help",cmd_help)]:
